...
describe("AuthController sign-in", () => {
  it("runs the slow canary verification path when the user does not exist", async () => {
    const { controller, authUsersRepository, passwordService } = createServiceAndController();
    ...
    await expect(controller.signIn(req.body as never, req, res)).rejects.toThrow();
  ...
  it("creates a session and auth cookies on successful sign-in", async () => {
    ...
    await controller.signIn(req.body as never, req, res);
