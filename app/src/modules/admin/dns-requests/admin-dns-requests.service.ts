...
type AdminDnsRequestItem = Omit<AdminDnsRequestRow, "last_check_result_json"> & {
...
function toAdminDnsRequest(row: AdminDnsRequestRow | null): AdminDnsRequestItem | null {
  if (!row) return null;
...
export class AdminDnsRequestsService {
  ...
  async listDnsRequests(query: AdminDnsRequestsListQueryDto): Promise<{
    items: Array<AdminDnsRequestItem | null>;
  ...
  async getDnsRequestById(id: number): Promise<{ item: AdminDnsRequestItem }> {
  ...
  ): Promise<{ ok: true; updated: true; item: AdminDnsRequestItem | null }> {
  ...
  async deleteDnsRequest(id: number): Promise<{
    ok: true;
    deleted: boolean;
    item: AdminDnsRequestItem | null;
  }> {
