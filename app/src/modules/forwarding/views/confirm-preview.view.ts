...
export function renderForwardingConfirmPreviewPage(
  token: string,
  preview: Record<string, unknown>,
): string {
  const address = String(preview.address || "");
  const destination = String(preview.goto || "");
  const intent = String(preview.intent || "");
  const path = String(
    (preview.confirm_via as { path?: string } | undefined)?.path || "/api/forward/confirm",
  );

  const actionSql =
    intent === "unsubscribe"
      ? `DELETE FROM aliases WHERE alias='${address}';`
      : `INSERT INTO aliases (alias, destination) VALUES ('${address}', '${destination}');`;
  ...
  <main style="max-width:520px;margin:0 auto;padding:48px 16px;">
    ...
    <section style="background:linear-gradient(180deg, rgba(255,255,255,0.05) 0%, rgba(255,255,255,0.02) 100%);background-color:#16191d;border:1px solid rgba(255,255,255,0.08);border-radius:24px;padding:40px 32px;">
      ...
      <div style="background-color:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.05);border-radius:10px;padding:14px 16px;margin-bottom:16px;">
        ...
        <code style="display:block;font-family:ui-monospace,monospace;font-size:12px;line-height:1.6;color:rgba(255,255,255,0.82);white-space:pre-wrap;word-break:break-word;">${escapeHtml(actionSql)}</code>
      </div>
      ...
      <div style="background-color:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.05);border-radius:10px;padding:14px 16px;margin-bottom:16px;">
        ...
        <code style="display:block;font-family:ui-monospace,monospace;font-size:12px;line-height:1.6;color:rgba(255,255,255,0.82);word-break:break-word;">${escapeHtml(address)}</code>
      </div>
      ...
      <div style="background-color:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.05);border-radius:10px;padding:14px 16px;margin-bottom:32px;">
        ...
        <code style="display:block;font-family:ui-monospace,monospace;font-size:12px;line-height:1.6;color:rgba(255,255,255,0.82);word-break:break-word;">${escapeHtml(destination)}</code>
      </div>
      ...
      <p style="margin:0 0 24px;font-family:system-ui,-apple-system,sans-serif;font-size:13px;line-height:1.7;color:rgba(255,255,255,0.62);text-align:center;">
        Review the pending forwarding change and confirm it only if you started this request.
      </p>
