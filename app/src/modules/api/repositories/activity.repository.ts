  ...
  async listByOwner(
    ...
    return this.database.query<ActivityRow[]>(
      `SELECT *
       FROM (
         SELECT
           JSON_UNQUOTE(JSON_EXTRACT(l.body, '$.operation')) AS type,
           l.created_at AS occurred_at,
           l.route AS route,
           NULL AS intent,
           JSON_UNQUOTE(JSON_EXTRACT(l.body, '$.resource.alias')) AS alias
         FROM api_logs l
         WHERE l.api_token_owner_email = ?
           AND JSON_UNQUOTE(JSON_EXTRACT(l.body, '$.outcome')) = 'success'
           AND JSON_UNQUOTE(JSON_EXTRACT(l.body, '$.operation')) IN ('alias_create', 'alias_delete')
