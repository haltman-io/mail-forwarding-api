...
export class CheckDnsService {
  ...
  requestUi(req: Request, target: string): Promise<RelayResult> {
    return this.relay(req, "POST /api/request/ui", target, () => this.client.requestUi(target));
  }

  requestEmail(req: Request, target: string): Promise<RelayResult> {
    return this.relay(req, "POST /api/request/email", target, () => this.client.requestEmail(target));
  }
