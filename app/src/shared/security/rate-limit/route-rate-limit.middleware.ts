...
export class RouteRateLimitMiddleware implements NestMiddleware {
  ...
  private resolveRules(req: Request): Rule[] {
    ...
    if (method === "GET" && path === "/api/forward/unsubscribe") {
      return [
        this.globalLimitRule(),
        {
          kind: "delay",
    ...
    if (path === "/api/forward/confirm" && (method === "GET" || method === "POST")) {
      return this.confirmRules(method === "GET" ? normalizeGetToken : normalizeBodyToken);
    }

    if (method === "POST" && path === "/api/request/ui") {
      return [
        this.globalLimitRule(),
        {
          kind: "limit",
    ...
    if (method === "POST" && path === "/api/request/email") {
      return [
        this.globalLimitRule(),
        {
          kind: "limit",
    ...
    if (path === "/api/credentials/confirm" && (method === "GET" || method === "POST")) {
    ...
    if (
      ...
      (method === "POST" &&
        ["/api/auth/refresh", "/api/auth/sign-out", "/api/auth/sign-out-all"].includes(path))
    ) {
      return [this.globalLimitRule()];
    }
  ...
  private credentialsConfirmRules(tokenResolver: (request: Request) => string): Rule[] {
    return [
      ...
      },
      {
        ...
        key: (request) => `cred_confirm:${tokenResolver(request) || "missing"}`,
      },
    ];
  }
