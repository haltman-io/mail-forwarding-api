...
/**
 * @fileoverview Admin login notification email service.
 */
...
function buildSubject({ host, email, occurredAtIso }) {
  const fallback = "Security alert: admin login | {host}";
  const template = String(config.adminLoginEmailSubject || fallback).trim() || fallback;
 ...
 */
async function sendAdminLoginNotificationEmail({ email, requestIpText, userAgent, occurredAt }) {
  ...
  const subject = buildSubject({ host, email: to, occurredAtIso });
...
<html lang="en">
  ...
  <body style="margin:0;padding:0;background-color:#09090b;">
    <div style="display:none;max-height:0;overflow:hidden;opacity:0;color:transparent;mso-hide:all;">
      Admin login detected for ${escapeHtml(to)} from IP ${escapeHtml(requestIp)} at ${escapeHtml(
      occurredAtIso
    )}.
    </div>
                  ...
                  <tr>
                    <td style="padding:20px;">
                      <p style="margin:0 0 14px;font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;font-size:14px;line-height:1.6;color:rgba(255,255,255,.86);">
                        An admin authentication event was completed successfully.
                      </p>
...
USER AGENT: ${escapeHtml(ua)}</code></pre>
                      ...
                      <p style="margin:16px 0 0;font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;font-size:13px;line-height:1.6;color:rgba(255,255,255,.62);">
                        If this action was not expected, rotate credentials immediately and review access logs.
                      </p>
                    </td>
                  </tr>
...
module.exports = { sendAdminLoginNotificationEmail };
