...
/**
 * @fileoverview Auth register email verification workflow.
 */
...
const { resolveConfirmBaseUrl } = require("./email-confirmation-service");
...
function getHostFromUrl(baseUrl) {
...
/**
 * Send register confirmation email and persist the one-time token.
 ...
 * @returns {Promise<{ ok: boolean, sent: boolean, reason?: string, ttl_minutes: number, pending?: object, action?: string }>}
 */
async function sendAuthRegisterEmail({
  email,
  passwordHash,
  requestIpText,
  userAgent,
  ...
  const ttlMinutes = getAuthRegisterTtlMinutes();
  const cooldownSeconds = getAuthRegisterCooldownSeconds();
  const maxSendCount = getAuthRegisterMaxSends();
  const requestIpPacked = requestIpText ? packIp16(requestIpText) : null;
  const ua = String(userAgent || "").slice(0, 255);

  const result = await authRegisterRequestsRepository.upsertPendingByEmailTx({
    email: to,
  ...
  if (result.action === "taken") {
    const err = new Error("user_taken");
    err.code = "user_taken";
    throw err;
  }
  ...
  const confirmUrl = buildRegisterConfirmUrl(token, confirmBaseUrl);

  const from = String(config.smtpFrom || "").trim();
  if (!from) throw new Error("missing_SMTP_FROM");

  const host = getHostFromUrl(confirmBaseUrl);
  ...
  const text =
    ...
    `ACCOUNT REGISTRATION REQUEST DETECTED.\n` +
    `STATUS: PENDING EMAIL VERIFICATION.\n\n` +
    `CONFIRM LINK\n` +
    `${confirmUrl}\n\n` +
    `VERIFY THIS EMAIL TO ACTIVATE THE ACCOUNT.\n` +
...
<html lang="en">
  ...
  <body style="margin:0;padding:0;background-color:#09090b;">
    <div style="display:none;max-height:0;overflow:hidden;opacity:0;color:transparent;mso-hide:all;">
      Your verification code is ${token}. Use it to activate your account. Expires in ${ttlMinutes} minutes.
    </div>
                  ...
                  <tr>
                    <td style="padding:20px;">
                      ...
                      <p style="margin:0 0 14px;font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;font-size:13px;line-height:1.6;color:rgba(255,255,255,.62);">
                        Use this code or the link below within
