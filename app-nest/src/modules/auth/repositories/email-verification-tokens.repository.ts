...
import { createOpaqueToken } from "../../../shared/utils/auth-secrets.js";
...
type UpsertResult =
  | { action: "not_needed"; pending: null }
...
export class EmailVerificationTokensRepository {
  ...
  async upsertPendingByUserIdTx(payload: {
    ...
    return withTxRetry(this.database, async (conn) => {
      const userRows = await conn.query<{ id: number; email_verified_at: Date | string | null }[]>(
  ...
  async consumePendingTokenTx(payload: {
    tokenHash32: Buffer;
  }): Promise<{ ok: boolean; reason?: string; user?: { id: number; username: string; email: string } }> {
    ...
    return withTxRetry(this.database, async (conn) => {
      ...
      const userRows = await conn.query<
        { id: number; username: string; email: string; email_verified_at: Date | string | null; is_active: number }[]
      >(
